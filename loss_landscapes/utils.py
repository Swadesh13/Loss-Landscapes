...
def setup_PCA_directions(model_files, w, device = torch.device("cuda" if torch.cuda.is_available() else "cpu")):
    """
    ...
    for model_file in model_files:
        net2 = torch.load(model_file, map_location=device)
...
def project_trajectory(dir1, dir2, w, model_files, proj_method='cos', device = torch.device("cuda" if torch.cuda.is_available() else "cpu")):
    """
