...
parser.add_argument(
    "--dataset", type=str, default="cifar10", choices=["cifar10", "cifar100", "celeba"]
)
...
parser.add_argument(
    "--loss", type=str, default="crossentropy", choices=["crossentropy"]
)
parser.add_argument(
    "--device",
    type=str,
...
parser.add_argument(
    "--pca", type=str, help="if pca, then provide --pca path/to/weights/dir"
)
parser.add_argument(
    "--h5", type=str, help="load from existing h5 and create visualizations"
)
...
if args.h5:
    if args.show_2d:
        ...
        create_3D_losscape(h5=args.h5, pca=args.pca)
...
if not args.pca:
    model = torch.load(args.ckpt, map_location=device).eval()

if args.pca:
    weights = glob.glob(os.path.join(args.pca, "*.pt"))
    weights.sort(key=lambda f: int(''.join(filter(str.isdigit, f))))
    model = torch.load(weights[-1], map_location=device).eval()
    kwarg = {
        "pca": weights,
        "y_min": args.ymin,
        "y_max": args.ymax,
